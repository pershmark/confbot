...
class ClickMeetingRestClient:
    ...
    def __init__(self, params):
        self.api_key = params['api_key'] if checkKey(params, 'api_key') else None

        formats = ['json', 'xml', 'js', 'printr']

        self.url = params['url'] if checkKey(params, 'url') else 'https://api.clickmeeting.com/v1/'

        self.format = params['format'].lower() if checkKey(params, 'format') and params['format'].lower() in formats else None
        ...
        def build_query_item(params, base_key=None):
            ...
            if(type(params).__name__ == 'dict'):
                for key, value in params.items():
                    if(base_key):
                        new_base = urllib.parse.quote(unicode("%s[%s]" % (base_key, key)))
                ...
                quoted_item = urllib.parse.quote(unicode(params))
                if(base_key):
